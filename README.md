...
* Ну и немного imho, если кому интересно. Позабавила меня ситуация с Mqtt  в Home Assistant. Оказывается, в нем когда-то был [встроенный сервер Mqtt](https://sprut.ai/article/home-assistant-statya-8-mqtt). Пишут, что были нарекания в его работе, но вместо доработки потом он был [убран из системы](https://github.com/home-assistant/architecture/issues/186). Для сравнения, тот же ioBroker, Mqtt сервер там не без нареканий. Лично я ушел с этой системы, ибо с некоторого релиза встроенный брокер решил сам определять, число ему приходит или строка. Когда слишком умный дом решает все сам и не дает возможности выброра, это тоже плохо. Тем не менее, сервер там остался. И есть выбор, пользоваться встроенным или использовать внешний брокер. Если уж в НА дело дошло до выпиливания встроенного Mqtt сервера (сейчас в документации на сайте не нашел даже упоминания о встроенном брокере), это заставляет задуматься о нежелании или неспособности разработчиков сделать нормальную поддержку этого стандарта (это при том, что по их же статистике Mqtt пользуется около 40% инсталляций),  и настороженно относиться к этой системе в целом. Особенно после их аргументов о лишней точке отказа или же о низкой пропускной способности. Учитывая, что узкое горлышко как раз на участке внешний брокер - Home Assistant. Тем не менее многие разделяют эту позицию. Мало того, в esp32 должна быть зашита только esphome прошивка. Спорить нет желания, каждый имеет право на свое мнение (и я тоже, как это ни странно!). Им нужно просто искать другие решения.
</details>     
...
&emsp;Шлюз также можно использовать для отслеживания до 24 BLE устройств меток/(маяков) со статическим MAC адресом. Выводится наличие/отсутствие метки(маяка) и rssi. Поддерживаются  BLE маяки приложения Home Assistant на смартфонах (привязка по uuid), а также термометры Xiaomi Mijia 2 с [прошивкой от atc1441 в режиме custom](https://github.com/atc1441/ATC_MiThermometer) и [прошивкой от pvvx в режиме custom](https://github.com/pvvx/ATC_MiThermometer).<br>
&emsp;Предусмотрено 5 портов ввода-вывода для управления внешними устройствами и чтения их состояния. Три четыре порта можно настроить как кнопки для включения - выключения подключенных BLE устройств, пятый порт - как кнопку обновления картинки с камеры. При конфигурации входов в режиме кнопок включается pullup, если это возможно (номер пина меньше 34).<br>
...
&emsp;Добавлено определение лучшего сигнала от метки(маяка) среди нескольких шлюзов. Алгоритм работает так. Каждый шлюз мониторит топик "r4s/DevId/rssi". Если у него сигнал от метки(маяка)  с большим уровнем, он прописывает свой уровень в этот топик, а также свой номер в топик "r4s/DevId/gtnum". После чего шлюз периодически, раз в 4 секунды сохраняет свой уровень сигнала в топике, то есть становится ведущим. Остальные шлюзы проверяют уровень и есть ли его обновление. Если какой-то шлюз обнаруживает отсутствие обновления уровня более 20 секунд, или же его уровень больше,  он становится ведущим. Лучший уровень и номер шлюза можно увидеть во второй строчке RSSI на странице BLE монитора. В сущностях каждого устройства тоже есть лучший уровень и номер шлюза:<br>
