...
Прошивается атом по usb без установки перемычек. Кнопку использовал для включения-выключения одного из устройств (39-Port1), светодиод пока в прошивке не задействован. Дальность особо не проверял, но устройство самое маленькое, даже обычная esp32 по размерам больше.
