...
* Ну и немного ффтопа, если кому интересно. Позабавила меня ситуация с Mqtt  в Home Assistant. Оказывается, в нем когда-то был [встроенный сервер Mqtt](https://sprut.ai/article/home-assistant-statya-8-mqtt). Пишут, что были нарекания в его работе, но вместо доработки потом он был [убран из системы](https://github.com/home-assistant/architecture/issues/186). Для сравнения, тот же ioBroker, Mqtt сервер там не без нареканий. Лично я ушел с этой системы, ибо с некоторого релиза встроенный брокер решил сам определять, число ему приходит или строка. Когда слишком умный дом решает все сам и не дает возможности выброра, это тоже плохо. Тем не менее, сервер там остался. И есть выбор, пользоваться встроенным или использовать внешний брокер. Если уж в НА дело дошло до выпиливания встроенного Mqtt сервера (сейчас на сайте не нашел даже упоминания о встроенном брокере), это заставляет задуматься о нежелании или неспособности разработчиков сделать нормальную поддержку этого стандарта (это при том, что по их же статистике Mqtt пользуется около 40% инсталляций),  и настороженно относиться к этой системе в целом. Особенно после аргументов о лишней точке отказа или же о низкой пропускной способности. Учитывая, что узкое горлышко как раз на участке внешний брокер - Home Assistant. 
</details>     
