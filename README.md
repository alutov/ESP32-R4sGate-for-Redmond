...
#### Изменения в текущей версии после последнего [релиза 2020.10.27](https://github.com/alutov/ESP32-R4sGate-for-Redmond/releases/tag/2020.10.27)
&emsp; Текущая версия 2020.11.07. Перешел с версии 4.2 на версию 4.3 esp-idf при компиляции бинарников. Вроде работает, но памяти стало кушать больше. Наблюдаю дальше.<br>
&emsp; Поправлена обработка температуры нагрева чайника heat_temp по Mqtt. Если задается температура до 40, чаник выключается, при 40..90 включается режим нагрева, при 91 и более включается режим кипячения. Добавлена возможность вывода в ответах на команды "true/false" вместо "ON/OFF". Мне пригодилась в в драйвере iot iobroker-а при подключении к яндекс колонке или гугл ассистенту. Они дружно понимают только "true/false". Позволяет не возиться в драйвере со скриптами автозамены. Опция включается в настройках и не работает, если включен Hass Discovery, т.к. в Home Assistant опция похоже не нужна, там "ON/OFF" идет по умолчанию. Увеличен размер стека BLE и http server-а. Доделал вроде бы совмещенные топики. В версиях от 2020.10.27 до 2020.11.02(пока тестил совмещенные топики), возможны проблемы с Mqtt. Если это так, стоит обновиться до текущей версии.<br>
