...
> **В версиях, начиная где-то с июня 2022, несколько раз менялся формат записи данных в NVS (Non-volatile storage - энергонезависимая память). Иногда это может привести к искажению данных в настройках после обновления. Возможны проблемы и с wifi и ble, так как их данные также хранятся в NVS. После обновления на последние версии следует зайти в настройки, проверить данные , затем отметить пункт "Format NVS area" и далее "Save setting". Память NVS  будет отформатирована и затем все данные заново перезаписаны.<br>**
...
&emsp;Шлюз также можно использовать для отслеживания до 24 BLE устройств меток/(маяков) со статическим MAC адресом. Выводится наличие/отсутствие метки(маяка) и rssi. Поддерживаются  BLE маяки приложения Home Assistant на смартфонах (привязка по uuid), термометры Xiaomi Mijia 2 с [прошивкой от atc1441 в режиме custom](https://github.com/atc1441/ATC_MiThermometer) и [прошивкой от pvvx в режиме custom](https://github.com/pvvx/ATC_MiThermometer), Xiaomi Mi Scale пока только v1, а также Samsung Smart Tag.<br>
...
<summary>Подробнее по Mqtt...</summary>
     У меня шлюз подключен к ioBroker. Мои настройки MQTT брокера ниже на картинке 4.<br>

![PROJECT_PHOTO](https://github.com/alutov/ESP32-R4sGate-for-Redmond/blob/master/jpg/mymqtt.jpg)
Картинка 4. Мои Mqtt настройки.<br><br>
...
#### Mqtt топики для чайника (см. картинку 5 ниже):
...
Картинка 5. Мои Mqtt топики для чайника.

#### Mqtt топики для мультиварки (см. картинку 6 ниже):
...
![PROJECT_PHOTO](https://github.com/alutov/ESP32-R4sGate-for-Redmond/blob/master/jpg/mymqtt2.jpg)
 Картинка 6. Мои Mqtt топики для мультиварки.
...
Настройки экрана для этой версии: 19-MISO, 23-MOSI, 18-CLK, 14-CS, 27-DC, 33-RES, 32-LED, 4-T_CS. Новые версии уже идут с экраном на ili9342. Начиная с версии 2021.10.29 добавлена поддержка экрана на ili9342. Я проверял работоспособность шлюза на новой версии M5Stack BASIC Kit. Прошивается он через встроенный USB разъем, перед прошивкой устройства нужно соединить последний контакт в верхнем ряду и 4 в нижнем ряду (gnd и gpio0) разъема (картинка 17). Возможна прошивка и без установки перемычек, зависит от программы. Настройки экрана для новой версии: 23-MISO, 23-MOSI, 18-CLK, 14-CS, 27-DC, 33-RES, 32-LED, 4-T_CS. Кнопки слева направо 39-Port1, 38-Port2, 37-Port3.<br>
