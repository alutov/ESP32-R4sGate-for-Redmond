...
* В MQTT устройства могут обмениваться данными и между собой, а не только с сервером умного дома. В шлюзе это используется для отображения на экране показаний основных датчиков в доме. Замечу сразу, что в нормальных брокерах существует система разделенного доступа (ACL).
