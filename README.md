...
* Ну и немного imho, если кому интересно. Понятно, что встроееный сервер в принципе гибче, надежнее и быстрее внешнего, гда сервер умного дома является лишь одним из клиентов. В Home Assistant не так давно тоже был [встроенный сервер Mqtt](https://sprut.ai/article/home-assistant-statya-8-mqtt), но потом он почему-то был [убран из системы](https://github.com/home-assistant/architecture/issues/186). Для сравнения, тот же ioBroker, Mqtt сервер там тоже не без проблем, но он там есть. Если уж в Home Assistant дело дошло до выпиливания встроенного Mqtt сервера (сейчас в документации на сайте не нашел даже упоминания о встроенном брокере), это очначает только одно - не смогли. Особенно после их аргументов о [лишней точке отказа, стабильности или же о больших задержках](https://esphome.io/components/api.html#advantages-over-mqtt). Давайте тогда сделаем Native API на внешнем http сервере, а потом уже посмотрим, кто кого. Тем не менее многие считают, что в esp32 должна быть зашита только esphome прошивка и общаться Home Assistant она должна по Native API. Но пока это не стандартный протокол и никем более не поддерживается, насколько мне известно. Спорить нет желания, каждый имеет право на свое мнение (и я тоже, как это ни странно!). Им нужно просто искать другие решения.
</details>     
