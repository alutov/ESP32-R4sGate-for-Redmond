...
> **Прошу перед использованием данных из этого проекта для начала прочитать описание. Несколько раз уже были запросы на реализацию уже действующих возможностей. Хочу также напомнить, что проект под лицензией MIT. Я не зарабатываю на проекте, просто делюсь тем, что уже работает у меня дома. Для русскоязычных перевожу: никто никому ничего не должен. Добавил бы следующее: если Вам уж так сильно хочется критиковать проект и его автора в интернете не по делу, а в принципе, пожалуйста, имейте совесть сначала прекратить пользоваться и удалить код проекта со всех своих устройств. К сожалению, этим грешат и уважаемые в среде умного дома люди. При необходимости могу уточнить, о ком идет речь.**<br>
...
Достаем из архива файлы btsnoop_hci.log и/или btsnoop_hci.log.last из папки "FS/data/log/bt/". В одном из этих файлов должен быть лог привязки. Далее в папку с этими файлами сохраняем архив с консольной утилитой stsk (https://github.com/alutov/ESP32-R4sGate-for-Redmond/raw/master/utils/stsk.zip) и достаем программу из архива. На всякий случай отмечу, что вирусов в архиве нет. Открываем командную сnроку windows, заходим в папку с файлами и набираем "stsk btsnoop_hci.log" или "stsk btsnoop_hci.log.last". Программа выведет найденные Smart Tag и сгенерирует ключи для них. Последний ключ будет скопирован в буфер обмена windows.<br>     
</details>
