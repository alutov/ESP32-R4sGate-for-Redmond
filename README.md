...
> **Прошивка использует Mqtt, что позволяет ей работать с практически всеми системами умного дома. Отображаемая на экране дополнительная информация также берется с Mqtt. Любителей ESPHome прошу не спамить в issues, а искать другие решения - портирование в ESPHome не планируется, так ка это ведет к потере функционала и, возможно, стабильности.**<br> 
...
* Добавлено вычисление количества воды в чайнике при нагреве в интервале 70-90&deg;C. Одновременно может служить индикатором кипевшей воды. Вычисляется на основе затраченной энергии и разности температур. КПД чайника изначально принят 80%. Точность так себе, у меня выходит где-то ~0.2л. Для повышения точности предусмотрен режим корректировки значения кпд. Для этого нужно залить в чайник 1л воды и выбрать в web-интерфейсе "Boil 1l on". Когда режим отработает, нужно зайти в режим настроек. Новое значение будет выведено сразу за типом устройства. Записать новое значение в nvram можно командой Save setting. 
