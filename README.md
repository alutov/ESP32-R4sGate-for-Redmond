...
* Ну и немного imho, если кому интересно. Позабавила меня ситуация с Mqtt  в Home Assistant. Оказывается, в нем когда-то был [встроенный сервер Mqtt](https://sprut.ai/article/home-assistant-statya-8-mqtt). Пишут, что были нарекания в его работе, но вместо доработки потом он был [убран из системы](https://github.com/home-assistant/architecture/issues/186). Для сравнения, тот же ioBroker, Mqtt сервер там не без нареканий, но там остался. И есть выбор, пользоваться встроенным или использовать внешний брокер. Если уж в Home Assistant дело дошло до выпиливания встроенного Mqtt сервера (сейчас в документации на сайте не нашел даже упоминания о встроенном брокере), это заставляет задуматься о нежелании или неспособности разработчиков сделать нормальную поддержку этого стандарта (это при том, что по их же статистике Mqtt пользуется около 40% инсталляций). Особенно после их аргументов о [лишней точке отказа, стабильности или же о больших задержках]([https://esphome.io/components/api.html](https://esphome.io/components/api.html#advantages-over-mqtt)). Давайте тогда сделаем Native API на внешнем http сервере, а потом уже посмотрим, кто кого. Тем не менее многие разделяют эту позицию. Мало того, в esp32 должна быть зашита только esphome прошивка. Спорить нет желания, каждый имеет право на свое мнение (и я тоже, как это ни странно!). Им нужно просто искать другие решения.
</details>     
