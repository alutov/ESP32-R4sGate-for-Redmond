...
* В MQTT устройства могут обмениваться данными и между собой, а не только с сервером умного дома. В шлюзе это используется для отображения на экране показаний основных датчиков в доме. На аргумент, что одни устройсва могут бесконтрольно переписывать данные других устройств замечу, что в нормальных брокерах существует система разделенного доступа (ACL). Хотя на вопрос, а зачем вообще заводить в систему устройства, которым нет доверия, у меня ответа нет. 
