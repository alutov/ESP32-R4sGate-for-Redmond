...
* 2022.04.29. Добавлена опция авторизации устройств с использованием MAC адреса шлюза. Позволяет при соответстующей настройке работать рядом двум и более шлюзам с одинаковыми параметрами. Для каждого шлюза придется заново перепривязать все подключенные к нему устройства. Добавлена поддержка [ATC_MiThermometer с прошивкой от atc1441 в режиме custom](https://github.com/atc1441/ATC_MiThermometer#advertising-format-of-the-custom-firmware). Пока не проверял. 
