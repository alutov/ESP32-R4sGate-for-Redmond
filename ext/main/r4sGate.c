...
****************************************************************
	...
	Lutov Andrey  Donetsk 2020.08.27
...
//        esp_log_buffer_hex(AP_TAG, char_elem_result, 44);
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        gl_profile_tab[PROFILE_A_APP_ID].rxchar_handle = char_elem_result[0].char_handle;
                        esp_ble_gattc_register_for_notify (gattc_if, gl_profile_tab[PROFILE_A_APP_ID].remote_bda, char_elem_result[0].char_handle);

                        gl_profile_tab[PROFILE_A_APP_ID].txchar_handle = char_elem_result[1].char_handle;
...
void MqState() {
	...
	if (!bStNl) esp_mqtt_client_publish(mqttclient, ldata, "false", 0, 1, 0);
        else if (bState == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "true", 0, 1, 0);
	bprevStNl = bStNl;
	}
...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,buff);
        ...
        strcat(bsend,"<tr><td style='min-width:150px;'>Version</td><td style='width:80%;'>2020.08.27</td></tr>");
...
static esp_err_t psetting_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"\" size=\"15\">WIFI SSID &emsp;<input type=\"password\" input name=\"swfpsw\" value=\"");
	...
	if (MQTT_SERVER[0]) strcat(bsend,MQTT_SERVER);
	strcat(bsend,"\"size=\"15\">MQTT Server &emsp;<input name=\"smqid\" value=\"");
	if (MQTT_USER[0]) strcat(bsend,MQTT_USER);
	strcat(bsend,"\"size=\"15\">MQTT Login &emsp;<input type=\"password\" input name=\"smqpsw\" value=\"");
	if (MQTT_PASSWORD[0]) strcat(bsend,MQTT_PASSWORD);
	strcat(bsend,"\"size=\"15\">MQTT Password</br>");
#ifdef USE_TFT
...
#endif
	strcat(bsend,"<h3>Device Setting</h3><br/><input name=\"sreqnam\" value=\"");
	...
	itoa(RgbR,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"255\" size=\"3\">NightLight Red &emsp;<input name=\"glight\" type=\"number\" value=\"");
	itoa(RgbG,buff,10);
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"255\" size=\"3\">NightLight Green &emsp;<input name=\"blight\" type=\"number\" value=\"");
	itoa(RgbB,buff,10);
	strcat(bsend,buff);
	...
	strcat(bsend,"\"size=\"64\">320*176 JPEG Uri<br>");
#endif
...
*/
	strcpy(buf2,"swfid");
	parsuri(buf1,WIFI_SSID,buf2,512,33);
	strcpy(buf2,"swfpsw");
	parsuri(buf1,WIFI_PASSWORD,buf2,512,65);
	strcpy(buf2,"smqsrv");
	parsuri(buf1,MQTT_SERVER,buf2,512,16);
	strcpy(buf2,"smqid");
	parsuri(buf1,MQTT_USER,buf2,512,16);
	strcpy(buf2,"smqpsw");
	parsuri(buf1,MQTT_PASSWORD,buf2,512,16);
	strcpy(buf2,"smtopp1");
	parsuri(buf1,MQTT_TOPP1,buf2,512,32);
	strcpy(buf2,"smtopp2");
	parsuri(buf1,MQTT_TOPP2,buf2,512,32);
	strcpy(buf2,"smtopp3");
	parsuri(buf1,MQTT_TOPP3,buf2,512,32);
	strcpy(buf2,"smtopp4");
	parsuri(buf1,MQTT_TOPP4,buf2,512,32);
	strcpy(buf2,"smtopp5");
	parsuri(buf1,MQTT_TOPP5,buf2,512,32);
	strcpy(buf2,"smtopp6");
	parsuri(buf1,MQTT_TOPP6,buf2,512,32);
	strcpy(buf2,"sreqnam");
	parsuri(buf1,REQ_NAME,buf2,512,16);
	strcpy(buf2,"rlight");
	...
	R4SNUM = atoi(buf3);
	strcpy(buf2,"smjpuri");
	parsuri(buf1,MyHttpUri,buf2,512,64);
// write nvs
...
// every 1s display time and date
	...
	if ((!f_update) && (tft_conn) && (t_before_us - t_jpg_us > 16000000)) {
#ifdef USE_TFT
	ret = tftjpg();
	if (ret) t_last_us = t_before_us;
#endif
