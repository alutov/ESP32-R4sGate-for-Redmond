...
void tfblestate()
{
	...
	if (btauthorize) {
	if (!bState) setTextColor(TFT_BLUE, TFT_BLACK);
	else if (bState == 254) setTextColor(TFT_DARKGREY, TFT_BLACK);
	else setTextColor(TFT_RED, TFT_BLACK);
	sumx += drawString(DEV_NAME, sumx, 224, 2);
	sumx += drawString(":", sumx, 224, 2);
        setTextColor(TFT_GREEN, TFT_BLACK);
	sumx += drawString(" Temp: ", sumx, 224, 2);
	if (bCtemp > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bCtemp > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	else if (bCtemp == 0) setTextColor(TFT_DARKGREY, TFT_BLACK);
	itoa(bCtemp,buff,10);
	sumx += drawString(buff, sumx, 224, 2);
	sumx += drawString("'", sumx, 224, 2);
        setTextColor(TFT_GREEN, TFT_BLACK);
	sumx += drawString(", Heat: ", sumx, 224, 2);
	if (bHtemp > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bHtemp > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	itoa(bHtemp,buff,10);
	sumx += drawString(buff, sumx, 224, 2);
	sumx += drawString("'", sumx, 224, 2);
        setTextColor(TFT_GREEN, TFT_BLACK);
	sumx += drawString(", Ble:", sumx, 224, 2);
	if (iRssi < -100) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (iRssi < -90) setTextColor(TFT_YELLOW, TFT_BLACK);
	itoa(iRssi,buff,10);
	sumx += drawString(buff, sumx, 224, 2);
	...
	if (DEV_NAME[0]) strcpy(buf,DEV_NAME);
	else strcpy(buf,"Ble");
	strcat(buf,": Offline");
	if (FND_NAME[0]) {
	strcat(buf,", found: ");
