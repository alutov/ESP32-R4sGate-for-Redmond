...
//
	...
	strcat(bufd,ptr->tBLEAddr);
	if (!fcommtp) strcat(bufd,"/rsp");
	strcat(bufd,"/volume\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"/status\"}");
...
//	ptr->bHeat = ptr->notifyData[10];
	...
	ptr->bCVol = (ptr->bS1Energy * 360 * 0.8) / (42 * ptr->bC1temp);	
        ptr->bS1Energy = 0;
        ptr->bC1temp = 0;
	}
...
void HtpDeVSett(uint8_t blenum, char* bsend) {
	...
	strcat(bsend,"value=\"95\">Mi-Unknown</option></select>Type &emsp;NightLight<input name=\"rlight");
	strcpy(buff,"a");
	buff[0] = buff[0] + blenum;
	strcat(bsend,buff);
