...
****************************************************************
	...
	Lutov Andrey  Donetsk 2020.09.15
...
//        ESP_LOGI(AP_TAG, "ESP_GATTC_REG_FOR_NOTIFY_EVT");
	...
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/name");
	esp_mqtt_client_publish(mqttclient, ldata, DEV_NAMEA, 0, 1, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	...
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/prog");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/mode");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/temp");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/set_hour");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/set_min");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/delay_hour");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/delay_min");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/cmd/warm");
	esp_mqtt_client_subscribe(mqttclient, ldata, 0);
	}
	}
...
//        ESP_LOGI(AP_TAG, "ESP_GATTC_REG_FOR_NOTIFY_EVT");
	...
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/name");
	esp_mqtt_client_publish(mqttclient, ldata, DEV_NAMEB, 0, 1, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/cmd/state");
...
//        ESP_LOGI(AP_TAG, "ESP_GATTC_REG_FOR_NOTIFY_EVT");
	...
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/name");
	esp_mqtt_client_publish(mqttclient, ldata, DEV_NAMEC, 0, 1, 0);
	strcpy(ldata,MQTT_BASE_TOPIC);
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/cmd/state");
...
bool mkSyncA() {
	t_SyncA = 0;
	if (btauthoriza && (DEV_TYPA > 1) && (DEV_TYPA < 16)) {
...
bool mkSyncB() {
	t_SyncB = 0;
	if (btauthorizb && (DEV_TYPB > 1) && (DEV_TYPB < 16)) {
	uint8_t data[] = { 0, 0, 0, 0, 0, 0, 0, 0};
	time_t now;
	time(&now);
	uint8_t ttz;
...
bool mkSyncC() {
	t_SyncC = 0;
	if (btauthorizc && (DEV_TYPC > 1) && (DEV_TYPC < 16)) {
	uint8_t data[] = { 0, 0, 0, 0, 0, 0, 0, 0};
	time_t now;
	time(&now);
	uint8_t ttz;
...
void mAsStatus() {
	...
	if (DEV_TYPA && ( DEV_TYPA < 16 ) && (retc == 20)) {
...
void mBsStatus() {
	...
	if (DEV_TYPB && ( DEV_TYPB < 16 ) && (retc == 20)) {
...
void mCsStatus() {
	...
	if (DEV_TYPC && ( DEV_TYPC < 16 ) && (retc == 20)) {
...
void MqAState() {
	...
	esp_mqtt_client_publish(mqttclient, ldata, cStatusA, 0, 1, 0);
	...
	if (!bStateA) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateA == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStateA = bStateA;
	}
	...
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/rsp/nightlight");
	if (!bStNlA) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateA == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStNlA = bStNlA;
	}
	...
	if (!bStateA) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        ...
        else if (bStateA == 1) esp_mqtt_client_publish(mqttclient, ldata, "setting", 0, 1, 0);
        else if (bStateA == 2) esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
        else if (bStateA == 4) esp_mqtt_client_publish(mqttclient, ldata, "warming", 0, 1, 0);
        else if (bStateA == 5) esp_mqtt_client_publish(mqttclient, ldata, "delayedstart", 0, 1, 0);
	bprevStateA = bStateA;
	}
	...
	strcat(ldata,"/");
	strcat(ldata,tBLEAddrA);
	strcat(ldata,"/rsp/warm");
	if (!bAwarmA) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bAwarmA == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevAwarmA = bAwarmA;
	}
...
void MqBState() {
	...
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/rsp/json");
	esp_mqtt_client_publish(mqttclient, ldata, cStatusB, 0, 1, 0);
	...
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/rsp/state");
	if (!bStateB) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateB == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStateB = bStateB;
	}
	...
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/rsp/nightlight");
	if (!bStNlB) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateB == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStNlB = bStNlB;
	}
	...
	if (!bStateB) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        ...
        else if (bStateB == 1) esp_mqtt_client_publish(mqttclient, ldata, "setting", 0, 1, 0);
        else if (bStateB == 2) esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
        else if (bStateB == 4) esp_mqtt_client_publish(mqttclient, ldata, "warming", 0, 1, 0);
        else if (bStateB == 5) esp_mqtt_client_publish(mqttclient, ldata, "delayedstart", 0, 1, 0);
	bprevStateB = bStateB;
	}
	...
	strcat(ldata,tBLEAddrB);
	strcat(ldata,"/rsp/warm");
	if (!bAwarmB) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bAwarmB == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevAwarmB = bAwarmB;
	}
...
void MqCState() {
	...
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/rsp/json");
	esp_mqtt_client_publish(mqttclient, ldata, cStatusC, 0, 1, 0);
	...
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/rsp/state");
	if (!bStateC) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateC == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStateC = bStateC;
	}
	...
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/rsp/nightlight");
	if (!bStNlC) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bStateC == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevStNlC = bStNlC;
	}
	...
	if (!bStateC) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        ...
        else if (bStateC == 1) esp_mqtt_client_publish(mqttclient, ldata, "setting", 0, 1, 0);
        else if (bStateC == 2) esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
        else if (bStateC == 4) esp_mqtt_client_publish(mqttclient, ldata, "warming", 0, 1, 0);
        else if (bStateC == 5) esp_mqtt_client_publish(mqttclient, ldata, "delayedstart", 0, 1, 0);
	bprevStateC = bStateC;
	}
	...
	strcat(ldata,tBLEAddrC);
	strcat(ldata,"/rsp/warm");
	if (!bAwarmC) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 0);
        else if (bAwarmC == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 0);
        else esp_mqtt_client_publish(mqttclient, ldata, "on", 0, 1, 0);
	bprevAwarmC = bAwarmC;
	}
...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"<h3>1 ");
       	if (tBLEAddrA[0] && DEV_TYPA) {
	(DEV_TYPA > 15)? strcat(bsend,"Cooker, ") : strcat(bsend,"Kettle, ");
	...
	strcat(bsend,"}<h3>2 ");
       	if (tBLEAddrB[0] && DEV_TYPB) {
	(DEV_TYPB > 15)? strcat(bsend,"Cooker, ") : strcat(bsend,"Kettle, ");
	strcat(bsend,"Address: ");
	...
	strcat(bsend,"}<h3>3 ");
       	if (tBLEAddrC[0] && DEV_TYPC) {
	(DEV_TYPC > 15)? strcat(bsend,"Cooker, ") : strcat(bsend,"Kettle, ");
	strcat(bsend,"Address: ");
	...
	if (DEV_NAMEC[0]) strcat(bsend,DEV_NAMEC);
        ...
        strcat(bsend,"<tr><td style='min-width:150px;'>Version</td><td style='width:80%;'>2020.09.15</td></tr>");
...
static esp_err_t pcfgdev1_get_handler(httpd_req_t *req)
{
	...
	(REQ_NAMEA[0])? strcat(bsend,REQ_NAMEA) : strcat(bsend,"Not defined");
	if ((DEV_TYPA > 0) && (DEV_TYPA < 16)) strcat(bsend," Kettle");
	...
	if (bStateA ==1) strcat(bsend,"selected ");
...
static esp_err_t pcfgdev2_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"</header><table class='normal'><h3>1 ");
	(REQ_NAMEB[0])? strcat(bsend,REQ_NAMEB) : strcat(bsend,"Not defined");
	if ((DEV_TYPB > 0) && (DEV_TYPB < 16)) strcat(bsend," Kettle");
	...
	if (bStateB ==1) strcat(bsend,"selected ");
        strcat(bsend,"value=\"13\">Set Program</option>");
...
static esp_err_t pcfgdev3_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"</header><table class='normal'><h3>1 ");
	(REQ_NAMEC[0])? strcat(bsend,REQ_NAMEC) : strcat(bsend,"Not defined");
	if ((DEV_TYPC > 0) && (DEV_TYPC < 16)) strcat(bsend," Kettle");
	...
	if (bStateC ==1) strcat(bsend,"selected ");
        strcat(bsend,"value=\"13\">Set Program</option>");
...
static esp_err_t psetting_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"\"size=\"15\">Password</br>");
#ifdef USE_TFT
