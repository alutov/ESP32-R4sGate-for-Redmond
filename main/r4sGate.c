...
//	Isscanning = false;
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        ESP_LOGI(AP_TAG, "start handle %X end handle %X current handle value %X", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
...
//            ESP_LOGI(AP_TAG, "Unknown service source");
	...
	ESP_LOGI(AP_TAG, "Rx char count = %d, handle = %X", count1, gl_profile_tab[blenum].rxchar_handle);
	ESP_LOGI(AP_TAG, "Tx char count = %d, handle = %X", count2, gl_profile_tab[blenum].txchar_handle);
...
//	if ((!memcmp(p_data->notify.value, "\x55\x00\xff\x01\xaa", 5))||(!memcmp(p_data->notify.value, "\x55\x00\xff\x02\xaa", 5))||(!memcmp(p_data->notify.value, "\x55\x01\xff\x02\xaa", 5))) {
	if ((p_data->notify.value[0] = 0x55) && (p_data->notify.value[2] = 0xff) && p_data->notify.value[3] && (p_data->notify.value[3] < 0x04) && (p_data->notify.value[4] = 0xaa)) {
...
//init bt
	...
	esp_read_mac(binblemac, ESP_MAC_BT);
        esp_log_buffer_hex(AP_TAG, binblemac, 8);
