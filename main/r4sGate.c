...
//
	...
	} else if ((BleDevStA.DEV_TYP > 15) && (BleDevStA.DEV_TYP < 64)) { 
	if (!BleDevStA.bState) {
	...
	} else if ((BleDevStB.DEV_TYP > 15) && (BleDevStB.DEV_TYP < 64)) { 
	if (!BleDevStB.bState) {
	...
	} else if ((BleDevStC.DEV_TYP > 15) && (BleDevStC.DEV_TYP < 64)) { 
	if (!BleDevStC.bState) {
...
//	if (!f_update && !Isscanning && !StartingScan && (!BleDevStA.btconnect || BleDevStA.btauthoriz) && (!BleDevStB.btconnect || BleDevStB.btauthoriz) && (!BleDevStC.btconnect || BleDevStC.btauthoriz)) {
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Cooker.warming\",\"icon\":\"mdi:pot-steam\",\"uniq_id\":\"warm_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Cooker_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Cooker.temp\",\"uniq_id\":\"temp_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Cooker_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
	...
	strcat(bufd,",\"temp_step\":\"1\",\"modes\":[\"off\",\"heat\"],\"hold_modes\":[\"OFF\",\"Groats\",\"Frying\",\"Steam\",\"Baking\",\"Stewing\"");
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Cooker.prog\",\"icon\":\"mdi:pot-steam\",\"uniq_id\":\"prog_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Cooker_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Cooker.hour\",\"icon\":\"mdi:timer\",\"uniq_id\":\"phour_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Cooker_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Cooker.min\",\"icon\":\"mdi:timer\",\"uniq_id\":\"pmin_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Cooker_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
...
//	Isscanning = false;
	...
	break;
        ...
        ESP_LOGI(AP_TAG, "REMOTE BDA %d:", blenum1);
        esp_log_buffer_hex(AP_TAG, p_data->open.remote_bda, sizeof(esp_bd_addr_t));
        esp_err_t mtu_ret = esp_ble_gattc_send_mtu_req (gattc_if, p_data->open.conn_id);
	ptr->btopen = true;
        if (mtu_ret){
            ESP_LOGE(AP_TAG, "Config MTU %d error, error code = %x", blenum1, mtu_ret);
        }
	...
	conerr = 1;
	ESP_LOGE(AP_TAG,"Config mtu %d failed, error status = %x", blenum1, param->cfg_mtu.status);
        }
//	ESP_LOGI(AP_TAG, "ESP_GATTC_CFG_MTU_EVT, Status %d, MTU %d, conn_id %d", param->cfg_mtu.status, param->cfg_mtu.mtu, param->cfg_mtu.conn_id);
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ESP_LOGI(AP_TAG, "SEARCH RES %d: conn_id = %x is primary service %d", blenum1, p_data->search_res.conn_id, p_data->search_res.is_primary);
...
//            ptr->get_server = true;
	...
	conerr = 1;
            ESP_LOGE(AP_TAG, "Search service %d failed, error status = %x", blenum1, p_data->search_cmpl.status);
            break;
        }
...
ESP_LOGI(AP_TAG, "Update char handle = %x", gl_profile_tab[blenum].update_handle);
		}
...
if (p_data->read.handle == 0x2a) {
        esp_gatt_status_t ret_status = esp_ble_gattc_read_char( gattc_if,
			...
			conerr = 1;
                    }  ESP_LOGI(AP_TAG, "Read_read_mcuver_req %d Ok", blenum1);
	}
*/
	break;
	...
	bin2hex(gl_profile_tab[blenum].remote_bda, ptr->tBLEAddr,6);
	...
	cipherCrypt(buff2, buff1, bufftab, 4);
        esp_gatt_status_t ret_status = esp_ble_gattc_write_char( gattc_if,
			...
			conerr = 1;
                    ...
                    }  ESP_LOGI(AP_TAG, "Write_auth_xi_ack %d Ok", blenum1);
	buff2[0] = 0x03;
	buff2[1] = 0x01;
        ret_status = esp_ble_gattc_write_char( gattc_if,
                    ...
                    }  ESP_LOGI(AP_TAG, "Write_setup_xi %d Ok", blenum1);
	buff2[0] = 0x00;
        ret_status = esp_ble_gattc_write_char( gattc_if,
                                  gl_profile_tab[blenum].conn_id,
                    ...
                    }  ESP_LOGI(AP_TAG, "Write_boil_xi %d Ok", blenum1);
	buff2[0] = 0x17;
        ret_status = esp_ble_gattc_write_char( gattc_if,
                                  gl_profile_tab[blenum].conn_id,
                    ...
                    }  ESP_LOGI(AP_TAG, "Write_time_xi %d Ok", blenum1);
...
//			conerr = 1;
                    ...
                    }  ESP_LOGI(AP_TAG, "Read_read_ver_req %d Ok", blenum1);
//*/
...
//            ESP_LOGI(AP_TAG, "GATTC_NOTIFY_EVT_0, Handle = %x, Notify value:",p_data->notify.handle);
        }else{
...
//        esp_log_buffer_hex(AP_TAG, p_data->notify.value, p_data->notify.value_len);
	...
	conerr = 1;
            ESP_LOGE(AP_TAG, "Write descr %d failed, error status = %x", blenum1, p_data->write.status);
	conerr = 1;
            break;
        }
	...
	write_char_data_len = 4;
        ...
        esp_gatt_status_t ret_status = esp_ble_gattc_write_char( gattc_if,
			...
			conerr = 1;
                    }  ESP_LOGI(AP_TAG, "Write_auth %d Ok", blenum1);
	if (ptr->DEV_TYP > 63) {
	ptr->xbtauth = 1;
	...
	cipherCrypt(write_char_data, write_char_crypt_data, bufftab, 12);
        esp_gatt_status_t ret_status = esp_ble_gattc_write_char( gattc_if,
			...
			conerr = 1;
                    }  ESP_LOGI(AP_TAG, "Write_auth_mi %d Ok", blenum1);
	}
        break;
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        if (p_data->write.status != ESP_GATT_OK){
	conerr = 1;
            ESP_LOGE(AP_TAG, "Write char %d failed, error status = %x", blenum1, p_data->write.status);
            break;
        }
//	if (ptr->btauthoriz) start_scan();
	start_scan();
        break;
...
ESP_LOGI(AP_TAG, "Send 1, Handle %X, Data:", SHandle);
...
ESP_LOGI(AP_TAG, "Send 2, Handle %X, Data:", SHandle);
...
ESP_LOGI(AP_TAG, "Send 3, Handle %X, Data:", SHandle);
esp_log_buffer_hex(AP_TAG, BleDevStC.sendData, BleDevStC.sendDataLen);
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ...
        uint32_t duration = 0; //30;
        if (BleDevStA.REQ_NAME[0] || BleDevStB.REQ_NAME[0] || BleDevStC.REQ_NAME[0]) {
//start scan only if not scanning, if not already starting scan or if not open in progress any connection 
	if (!Isscanning && !StartingScan && (!BleDevStA.btconnect || BleDevStA.btopen) && (!BleDevStB.btconnect || BleDevStB.btopen) && (!BleDevStC.btconnect || BleDevStC.btopen)) {
        esp_ble_gap_start_scanning(duration);
        StartingScan = true;
	ESP_LOGI(AP_TAG, "Scan starting");
	}
	...
	bin2hex(scan_result->scan_rst.bda, FND_ADDR,6);
...
//                if ((BleDevStA.REQ_NAME[0]) &&  strlen(BleDevStA.REQ_NAME) == adv_name_len && !strncmp((char *)adv_name, BleDevStA.REQ_NAME, adv_name_len)) {
			if (FND_NAME[0]) strcpy (BleDevStA.RQC_NAME,FND_NAME);
                    ...
                    ESP_LOGI(AP_TAG, "Searched 0 device %s\n", BleDevStA.RQC_NAME);
...
//                else if ( (BleDevStC.REQ_NAME[0]) &&  strlen(BleDevStC.REQ_NAME) == adv_name_len && !strncmp((char *)adv_name, BleDevStC.REQ_NAME, adv_name_len)) {
	...
	if (!BleDevStA.btopen && BleDevStA.btconnect) {
                        ESP_LOGI(AP_TAG, "Connect 0 to the remote device");
...
void msStatus(uint8_t blenum) {
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"]}");    
	} else if ((ptr->DEV_TYP > 1) && ( ptr->DEV_TYP < 10 ) && (retc == 20)) {
...
//	ptr->bHeat = ptr->notifyData[10];
	...
	if (ptr->bState && (ptr->bCtemp > 70) && (ptr->bCtemp < 80) && !ptr->bC1temp && !ptr->bS1Energy) { 
	ptr->bS1Energy = ptr->bSEnergy;
	ptr->bC1temp = ptr->bCtemp;
	} else if (ptr->bState && (ptr->bCtemp > 90) && ptr->bC1temp && ptr->bS1Energy) {
...
//	ptr->bCVol = (ptr->bS1Energy * 360 * 0.8) / (42 * ptr->bC1temp); //if n=0.8 then L*10 = 10*(En*3600*0.8)/(4200*dT); 	
	...
	} else if (( ptr->DEV_TYP > 9) && ( ptr->DEV_TYP < 12 ) && (retc == 20)) {
	ptr->bHeat = 0;
	ptr->bState = ptr->notifyData[11];
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"}");    
	} else if (( ptr->DEV_TYP > 11 ) && ( ptr->DEV_TYP < 16 ) && (retc == 20)) {
	ptr->bHeat = 0;
	ptr->bState = ptr->notifyData[11];
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"}");    
	} else if (( ptr->DEV_TYP == 16 ) && (retc == 13)) {
	ptr->bHeat = 0;
	ptr->bProg = ptr->notifyData[3];
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"}");    
	} else if (( ptr->DEV_TYP > 16 ) && ( ptr->DEV_TYP < 24 ) && (retc == 20)) {
	ptr->bHeat = 0;
	ptr->bProg = ptr->notifyData[3];
	ptr->bModProg = ptr->notifyData[4];
	ptr->bHtemp = ptr->notifyData[5];
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"}");    
	} else if (( ptr->DEV_TYP == 24 ) && (retc == 20)) {
	ptr->bHeat = 0;
	ptr->bProg = ptr->notifyData[3];
	ptr->bModProg = ptr->notifyData[4];
	ptr->bHtemp = ptr->notifyData[5];
...
//	ptr->PRgbB = ptr->RgbB;
	...
	if (!fcommtp) strcat(ldata,"/rsp");
	strcat(ldata,"/hstate");
	if (ptr->bState < 2) esp_mqtt_client_publish(mqttclient, ldata, "off", 0, 1, 1);
        else if (ptr->bState == 254) esp_mqtt_client_publish(mqttclient, ldata, "offline", 0, 1, 1);
        else esp_mqtt_client_publish(mqttclient, ldata, "heat", 0, 1, 1);
	ptr->r4sppcom = 255;
	ptr->t_ppcom_us = t_mqt_us;
...
void BleMqtPr(uint8_t blenum, int topoff, char *topic, int topic_len, char *data, int data_len) {
	...
	ptr->r4slpcom = 4;
	if (ptr->bCVol == 253) ptr->bCVol = 254;
	} else {
	ptr->r4slppar1 = 0;
	ptr->r4slpcom = 2;
	if (ptr->bCVol == 253) ptr->bCVol = 254;
...
//	ESP_LOGI(AP_TAG,"MQTT_strength_OFF");
	...
	if ((!incascmp("1",data,data_len)) || (!incascmp("on",data,data_len))
		|| (!incascmp("true",data,data_len)) || (!incascmp("heat",data,data_len))) {
	if ((!ptr->bState) || (!ptr->r4sppcom) || (!fcommtp)) {
	ptr->r4slppar1 = 1;
	ptr->r4slpcom = 10;
	}
//	ESP_LOGI(AP_TAG,"MQTT_CMD_ON");
...
//
	...
	} else if (topoffa && BleDevStA.DEV_TYP && ((t_mqt_us - BleDevStA.t_ppcon_us) > 3000000)) {
...
//	BleMqtPr(uint8_t blenum, int topoff, char *topic, int topic_len, char *data, int data_len) {
	...
	} else if (topoffb && BleDevStB.DEV_TYP && ((t_mqt_us - BleDevStB.t_ppcon_us) > 3000000)) {
//ESP_LOGI(AP_TAG,"topoffb=%d", topoffb);
	BleMqtPr(1,  topoffb, event->topic, event->topic_len, event->data, event->data_len);

	} else if (topoffc && BleDevStC.DEV_TYP && ((t_mqt_us - BleDevStC.t_ppcon_us) > 3000000)) {
...
void MnHtpBleSt(uint8_t blenum, char* bsend) {
	...
	else if (ptr->DEV_TYP < 12) strcat(bsend,"Power, ");
...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,buff);
        strcat(bsend," bytes");
        if ((BleDevStA.REQ_NAME[0] && !BleDevStA.btauthoriz) || (BleDevStB.REQ_NAME[0] && !BleDevStB.btauthoriz) ||(BleDevStC.REQ_NAME[0] && !BleDevStC.btauthoriz)) {
        strcat(bsend,"</td></tr><tr><td>BLE last found name / address</td><td>");
...
void HtpDeVHandle(uint8_t blenum, char* bsend) {
	...
	strcat(bsend,buff);
	strcat(bsend,"\" min=\"0\" max=\"255\" size=\"3\">Light Blue</br></br>");
	} else if ( ptr->DEV_TYP > 63) {
...
*/
	...
	BleDevStA.r4slppar1 = 0;
	BleDevStA.r4slpcom = 2;
	if (BleDevStA.bCVol == 253) BleDevStA.bCVol = 254;
	break;
	case 3:             //heat(on)
	cm_done = 1;
	...
	BleDevStA.r4slppar1 = 0;
	BleDevStA.r4slpcom = 2;
	if (BleDevStA.DEV_TYP > 3) BleDevStA.bCVol = 253;
	break;
	case 64:             //mi off
	cm_done = 1;
	BleDevStA.r4slppar1 = 0;		
...
*/
	...
	BleDevStB.r4slppar1 = 0;
	BleDevStB.r4slpcom = 2;
	if (BleDevStB.bCVol == 253) BleDevStB.bCVol = 254;
	break;
	case 3:             //heat(on)
	cm_done = 1;
	...
	BleDevStB.r4slppar1 = 0;
	BleDevStB.r4slpcom = 2;
	if (BleDevStB.DEV_TYP > 3) BleDevStB.bCVol = 253;
	break;
	case 64:             //mi off
	cm_done = 1;
	BleDevStB.r4slppar1 = 0;		
...
*/
	...
	BleDevStC.r4slppar1 = 0;
	BleDevStC.r4slpcom = 2;
	if (BleDevStC.bCVol == 253) BleDevStC.bCVol = 254;
	break;
	case 3:             //heat(on)
	cm_done = 1;
	...
	BleDevStC.r4slppar1 = 0;
	BleDevStC.r4slpcom = 2;
	if (BleDevStC.DEV_TYP > 3) BleDevStC.bCVol = 253;
	break;
	case 64:             //mi off
	cm_done = 1;
	BleDevStC.r4slppar1 = 0;		
...
//r4s state nonitoring and command execution loop
	...
	t_tinc_us = t_before_us;	
        if (ble_mon && !f_update && BleDevStA.REQ_NAME[0] && BleDevStA.NumConn && !BleDevStA.btauthoriz) {
	if (floop) floop--;
	} else if (ble_mon && !f_update && BleDevStB.REQ_NAME[0] && BleDevStB.NumConn && !BleDevStB.btauthoriz) {
	if (floop) floop--;
	} else if (ble_mon && !f_update && BleDevStC.REQ_NAME[0] && BleDevStC.NumConn && !BleDevStC.btauthoriz) {
	if (floop) floop--;
