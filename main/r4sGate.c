...
//step 1: start scan only if not scanning, if not already starting scan or if not open in progress any connection 
...
bool mkSyncA() {
	t_SyncA = 0;
	if (btauthoriza && (DEV_TYPA > 2) && (DEV_TYPA < 64) && ((DEV_TYPA < 16) || (DEV_TYPA > 23))) {
...
bool mkSyncB() {
	t_SyncB = 0;
	if (btauthorizb && (DEV_TYPB > 2) && (DEV_TYPB < 64) && ((DEV_TYPB < 16) || (DEV_TYPB > 23))) {
	uint8_t data[] = { 0, 0, 0, 0, 0, 0, 0, 0};
	time_t now;
	time(&now);
	uint8_t ttz;
...
bool mkSyncC() {
	t_SyncC = 0;
	if (btauthorizc && (DEV_TYPC > 2) && (DEV_TYPC < 64) && ((DEV_TYPC < 16) || (DEV_TYPC > 23))) {
	uint8_t data[] = { 0, 0, 0, 0, 0, 0, 0, 0};
	time_t now;
	time(&now);
	uint8_t ttz;
...
//	bprevStateA = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 29) && (temp < 91) && ((!fcommtp) || (!r4sppcoma) || (temp != bHtempA))) {
	r4slppar1a = temp;
	r4slpcoma = 3;
...
//	bprevStateB = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 29) && (temp < 91) && ((!fcommtp) || (!r4sppcomb) || (temp != bHtempB))) {
	r4slppar1b = temp;
	r4slpcomb = 3;
...
//	bprevStateC = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 29) && (temp < 91) && ((!fcommtp) || (!r4sppcomc) || (temp != bHtempC))) {
	r4slppar1c = temp;
	r4slpcomc = 3;
...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,buff);
        ...
        if (!Isscanning) strcat(bsend,"Scanning");
	else if (btconnecta && !btopena) {
	strcat(bsend,"Connecting ");
	strcat(bsend, REQ_NAMEA);
	} else if (btconnectb && !btopenb) {
	strcat(bsend,"Connecting ");
	strcat(bsend, REQ_NAMEB);
	} else if (btconnectc && !btopenc) {
	strcat(bsend,"Connecting ");
