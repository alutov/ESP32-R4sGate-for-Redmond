...
//	ESP_LOGI(AP_TAG,"sent publish successful, msg_id=%d", msg_id);
	...
	if (MQTT_TOPP6[0]) esp_mqtt_client_subscribe(client, MQTT_TOPP7, 0);
#endif
...
//	blstnum_inc();
	...
	blstnum_inc();
	t_tinc_us = t_before_us;	
        if (!f_update && REQ_NAMEA[0] && NumConnA && !btauthoriza) {
	if (floop) floop--;
	} else if (!f_update && REQ_NAMEB[0] && NumConnB && !btauthorizb) {
	if (floop) floop--;
	} else if (!f_update && REQ_NAMEC[0] && NumConnC && !btauthorizc) {
	if (floop) floop--;
	} else floop = 16;
