...
//
	...
	strcat(bufd,tBLEAddrA);
	if (!fcommtp) strcat(bufd,"/rsp");
	strcat(bufd,"/total_energy\",\"unit_of_meas\":\"kW*h\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,tBLEAddrA);
	strcat(bufd,"/status\"}");
...
bool mA171sHeat(uint8_t temp, uint8_t start) {
	return mA171sOn(1, temp);
}

bool mB171sHeat(uint8_t temp, uint8_t start) {
	return mB171sOn(1, temp);
}

bool mC171sHeat(uint8_t temp, uint8_t start) {
...
//r4s state nonitoring and command execution loop
	...
	case 2:             //boil
	r4slpresa = 1;
	if ((DEV_TYPA < 10) && bHtempA && (bHtempA < 91)) mA171sHeat(0,1);
	mA171sOff();
	bprevHtempA = 255;
	bprevStateA = 255;
	bprevHeatA = 255;
	...
	bprevHeatA = 255;
	bprevStNlA = 255;
	mA171sHeat(r4slppar1a,r4slppar2a);
	r4slpcoma = 0;
	t_lasta_us = ~t_lasta_us;
	break;
	case 4:             //boil&heat
...
// every 1s display time and date
	...
	case 2:             //boil
	r4slpresb = 1;
	if ((DEV_TYPB < 10) && bHtempB && (bHtempB < 91)) mB171sHeat(0,1);
	mB171sOff();
	bprevHtempB = 255;
	bprevStateB = 255;
	bprevHeatB = 255;
	...
	bprevHeatB = 255;
	bprevStNlB = 255;
	mB171sHeat(r4slppar1b,r4slppar2b);
	r4slpcomb = 0;
	t_lastb_us = ~t_lastb_us;
	break;
	case 4:             //boil&heat
...
// every 1s display time and date
	...
	case 2:             //boil
	r4slpresc = 1;
	if ((DEV_TYPC < 10) && bHtempC && (bHtempC < 91)) mC171sHeat(0,1);
	mC171sOff();
	bprevHtempC = 255;
	bprevStateC = 255;
	bprevHeatC = 255;
	...
	bprevHeatC = 255;
	bprevStNlC = 255;
	mC171sHeat(r4slppar1c,r4slppar2c);
	r4slpcomc = 0;
	t_lastc_us = ~t_lastc_us;
	break;
	case 4:             //boil&heat
