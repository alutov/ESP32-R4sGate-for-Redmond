...
//	strcat(bufd,tESP32Addr);
	...
	strcat(bufd,tBLEAddrA);
	if (!fcommtp) strcat(bufd,"/rsp");
	strcat(bufd,"/total_energy\",\"unit_of_meas\":\"W*h\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,tBLEAddrA);
	strcat(bufd,"/status\"}");
...
void mAsStatus() {
	...
	(notifyDataA[3])? strcat(cStatusA,"1") : strcat(cStatusA,"0");
	...
	if (DEV_TYPA > 3) bCtempA = notifyDataA[8];
	...
	if ((bStateA == 4) || bProgA) bStateA = 0;
	if (DEV_TYPA > 3) bStBpA = notifyDataA[7];
	else bStBpA = 0;
	bStBlA = 0;
	bStBlA = 0;
	strcpy(cStatusA,"{\"temp\":");
	...
	strcat(cStatusA,tmpvar);
	strcat(cStatusA,"]}");    
	if (btauthoriza && (DEV_TYPA > 3)) {
	uint8_t data[] = {0xc8};
	if (r4sCommandA(0x35, data, sizeof(data)) == 9) bStBlA = notifyDataA[5];
	data[0] = 0x00;
...
void mBsStatus() {
	...
	(notifyDataB[3])? strcat(cStatusB,"1") : strcat(cStatusB,"0");
	...
	if (DEV_TYPB > 3) bCtempB = notifyDataB[8];
	...
	if ((bStateB == 4) || bProgB) bStateB = 0;
	if (DEV_TYPB > 3) bStBpB = notifyDataB[7];
	else bStBpB = 0;
	bStBlB = 0;
	strcpy(cStatusB,"{\"temp\":");
	...
	strcat(cStatusB,tmpvar);
	strcat(cStatusB,"]}");    
	if (btauthorizb && (DEV_TYPB > 3)) {
	uint8_t data[] = {0xc8};
	if (r4sCommandB(0x35, data, sizeof(data)) == 9) bStBlB = notifyDataB[5];
	data[0] = 0x00;
...
void mCsStatus() {
	...
	(notifyDataC[3])? strcat(cStatusC,"1") : strcat(cStatusC,"0");
	...
	if (DEV_TYPC > 3) bCtempC = notifyDataC[8];
	...
	if ((bStateC == 4) || bProgC) bStateC = 0;
	if (DEV_TYPC > 3) bStBpC = notifyDataC[7];
	else bStBpC = 0;
	bStBlC = 0;
	strcpy(cStatusC,"{\"temp\":");
	...
	strcat(cStatusC,tmpvar);
	strcat(cStatusC,"]}");    
	if (btauthorizc && (DEV_TYPC > 3)) {
	uint8_t data[] = {0xc8};
	if (r4sCommandC(0x35, data, sizeof(data)) == 9) bStBlC = notifyDataC[5];
	data[0] = 0x00;
...
//	ESP_LOGI(AP_TAG,"MQTT_BOIL_OFF");
	...
	if  ((temp > 29) && (temp < 99) && ((!fcommtp) || (!r4sppcoma) || (temp != bHtempA))) {
	if (DEV_TYPA == 1) {	
	if (temp < 41) r4slppar1a = 1;
...
//	ESP_LOGI(AP_TAG,"MQTT_BOIL_OFF");
	...
	if  ((temp > 29) && (temp < 99) && ((!fcommtp) || (!r4sppcomb) || (temp != bHtempB))) {
	if (DEV_TYPB == 1) {	
	if (temp < 41) r4slppar1b = 1;
...
//	ESP_LOGI(AP_TAG,"MQTT_BOIL_OFF");
	...
	if  ((temp > 29) && (temp < 99) && ((!fcommtp) || (!r4sppcomc) || (temp != bHtempC))) {
	if (DEV_TYPC == 1) {	
	if (temp < 41) r4slppar1c = 1;
...
//	strcat(bsend,"value=\"66\">V-SK152(Int)</option></select>Type &emsp;NightLight");
	...
	strcat(bsend,"> BLE monitoring</br>");
