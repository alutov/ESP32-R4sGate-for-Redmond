...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend, REQ_NAMEA);
        (btopena)? strcat(bsend," (Auth") : strcat(bsend," (Open");
	if (DEV_TYPA > 63) {
	strcat(bsend,"/Id:");
	...
	strcat(bsend, REQ_NAMEB);
        (btopenb)? strcat(bsend," (Auth") : strcat(bsend," (Open");
	if (DEV_TYPB > 63) {
	strcat(bsend,"/Id:");
	...
	strcat(bsend, REQ_NAMEC);
        (btopenc)? strcat(bsend," (Auth") : strcat(bsend," (Open");
	if (DEV_TYPC > 63) {
	strcat(bsend,"/Id:");
