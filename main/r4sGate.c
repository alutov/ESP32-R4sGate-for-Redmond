...
*/
        if (!f_update && !Isscanning && !StartingScan) {
	...
	} else if (ble_mon == 1) scan_ret = esp_ble_gap_set_scan_params(&ble_pscan_params);
        if (scan_ret){
...
//	strcat(bufd,"\",\"manufacturer\":\"Redmond\"},\"device_class\":\"pressure\",\"state_class\":\"measurement\",\"state_topic\":\"");
	...
	strcat(bufd,ptr->tBLEAddr);
	if (!fcommtp) strcat(bufd,"/rsp");
	strcat(bufd,"/pressure\",\"unit_of_meas\":\"mmHg\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"/status\"}");
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
       	itoa(blenum1,tbuff,10);
	strcat(bufd,tbuff);
	strcat(bufd,".Weather.quality\",\"icon\":\"mdi:air-filter\",\"uniq_id\":\"qual_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\",\"device\":{\"identifiers\":[\"Weather_");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"\"],\"name\":\"");
...
//	strcat(bufd,"\",\"manufacturer\":\"Redmond\"},\"device_class\":\"aqi\",\"state_class\":\"measurement\",\"state_topic\":\"");
	...
	strcat(bufd,ptr->tBLEAddr);
	if (!fcommtp) strcat(bufd,"/rsp");
	strcat(bufd,"/quality\",\"unit_of_meas\":\"ppb\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/");
	strcat(bufd,ptr->tBLEAddr);
	strcat(bufd,"/status\"}");
...
//	if ((!memcmp(p_data->notify.value, "\x55\x00\xff\x01\xaa", 5))||(!memcmp(p_data->notify.value, "\x55\x00\xff\x02\xaa", 5))||(!memcmp(p_data->notify.value, "\x55\x01\xff\x02\xaa", 5))) {
	if ((p_data->notify.value[0] = 0x55) && (p_data->notify.value[2] = 0xff) && (p_data->notify.value[3] < 0x04) && (p_data->notify.value[4] = 0xaa)) {
...
//	if (ptr->DEV_TYP == 63) write_char_data[1] = 1;
...
//	ESP_LOGI(AP_TAG, "searched Adv Data Len %d, Scan Response Len %d", scan_result->scan_rst.adv_data_len, scan_result->scan_rst.scan_rsp_len);
	...
	ESP_LOGI(AP_TAG, "Rssi %d dBm, searched Device Name Len %d", scan_result->scan_rst.rssi, adv_name_len);
...
//	ptr->bHeat = ptr->notifyData[10];
	...
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,"}");    
	} else if (( ptr->DEV_TYP == 63 ) && (retc == 20)) {
	ptr->bSEnergy = (ptr->notifyData[4] + (ptr->notifyData[5] << 8)) & 0x0fff;      //temp C
	ptr->bCtemp = ptr->notifyData[9];  //humidity %
	ptr->bSTime = ptr->notifyData[10] + (ptr->notifyData[11] << 8);      //pressure mmHg
	ptr->bSCount = ptr->notifyData[13] + (ptr->notifyData[14] << 8);      //quality ppb
	strcpy(ptr->cStatus,"{\"temperature\":");
	itoa(ptr->bSEnergy / 10,tmpvar,10);
	strcat(ptr->cStatus,tmpvar);
	strcat(ptr->cStatus,".");
	itoa(ptr->bSEnergy % 10,tmpvar,10);
	strcat(ptr->cStatus,tmpvar);
...
//	ptr->bprevModProg = ptr->bModProg;
	...
	itoa(ptr->bSEnergy / 10,tmpvar1,10);
	strcpy(tmpvar,tmpvar1);
	strcat(tmpvar,".");	
	itoa(ptr->bSEnergy % 10,tmpvar1,10);
	strcat(tmpvar,tmpvar1);
...
//	esp_mqtt_client_publish(mqttclient, llwtt, llwtd, 0, 1, 1);
	...
	strcat(llwtd,".Gate.BleDev");
       	itoa(i+1,tmpvar,10);
	strcat(llwtd,tmpvar);
	strcat(llwtd,".state\",\"icon\":\"mdi:bluetooth\",\"uniq_id\":\"bdev");
       	itoa(i+1,tmpvar,10);
	strcat(llwtd,tmpvar);
	strcat(llwtd,"state_");
	strcat(llwtd,tESP32Addr);
...
static esp_err_t pblemon_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"value=\"1\">On</option></select>");
	strcat(bsend,"</th><th align='left''></th><th align='left''></th><th align='left''></th><th align='left'></th><th align='left''></tr>");
