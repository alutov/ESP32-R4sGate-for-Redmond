...
void MqttPubSubA (bool mqtttst) {
	if (tBLEAddrA[0] && mqtttst && DEV_TYPA) {
	char buft[64];
	char bufd[2048];
...
//
	...
	strcat(bufd,",\"temp_step\":\"1\",\"modes\":[\"off\",\"heat\"],\"hold_modes\":[\"OFF\",\"Milk_porridge\",\"Stewing\",\"Frying\",\"Soup\"");
	strcat(bufd,",\"Steam\",\"Pasta\",\"Slow_cooking\",\"Hot\",\"Baking\"");
	strcat(bufd,",\"Groats\",\"Pilaf\",\"Yogurt\",\"Pizza\",\"Bread\",\"Desserts\"]}");
	}
	esp_mqtt_client_publish(mqttclient, buft, bufd, 0, 1, 1);
//
	strcpy(buft,"homeassistant/sensor/");
...
void MqttPubSubB (bool mqtttst) {
	if (tBLEAddrB[0] && mqtttst && DEV_TYPB) {
	char buft[64];
	char bufd[2048];
	strcpy(buft,MQTT_BASE_TOPIC);
	strcat(buft,"/");
...
void MqttPubSubC (bool mqtttst) {
	if (tBLEAddrC[0] && mqtttst && DEV_TYPC) {
	char buft[64];
	char bufd[2048];
	strcpy(buft,MQTT_BASE_TOPIC);
	strcat(buft,"/");
...
/*
        ret_status = esp_ble_gattc_read_char( gattc_if,
                                  gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                                  gl_profile_tab[PROFILE_A_APP_ID].ver_handle,
                                  ESP_GATT_AUTH_REQ_NONE);
                    if (ret_status != ESP_GATT_OK){
			conerr = 1;
...
//	if (conerr && btopena) esp_ble_gattc_close(gl_profile_tab[PROFILE_A_APP_ID].gattc_if,gl_profile_tab[PROFILE_A_APP_ID].conn_id);
	if (conerr && btopena) esp_ble_gap_disconnect(gl_profile_tab[PROFILE_A_APP_ID].remote_bda);
...
//	if (conerr && btopenb) esp_ble_gattc_close(gl_profile_tab[PROFILE_B_APP_ID].gattc_if,gl_profile_tab[PROFILE_B_APP_ID].conn_id);
	if (conerr && btopenb) esp_ble_gap_disconnect(gl_profile_tab[PROFILE_B_APP_ID].remote_bda);
}
//*** B ************************
...
//	if (conerr && btopenc) esp_ble_gattc_close(gl_profile_tab[PROFILE_C_APP_ID].gattc_if,gl_profile_tab[PROFILE_C_APP_ID].conn_id);
	if (conerr && btopenc) esp_ble_gap_disconnect(gl_profile_tab[PROFILE_C_APP_ID].remote_bda);
}
//*** C ************************
...
// for RMC-800s
	...
	case 7:		//milk_porridge молочна€ каша
	data[1] = 0;     	//mode
	data[2] = 95;		//temp
	data[3] = 0;		//shour
	data[4] = 35;		//smin
...
// for RMC-903s
	switch (prog) {
	case 1:		//milk_porridge молочна€ каша
	data[1] = 0;     	//mode
	data[2] = 97;		//temp
	data[3] = 0;		//shour
	data[4] = 10;		//smin
...
void mAsStatus() {
	...
	strcat(cStatusA,tmpvar);
	strcat(cStatusA,"}");    
	} else if (( DEV_TYPA > 15 ) && ( DEV_TYPA < 18 ) && (retc == 13)) {
	bProgA = notifyDataA[3];
...
void mBsStatus() {
	...
	strcat(cStatusB,tmpvar);
	strcat(cStatusB,"}");    
	} else if (( DEV_TYPB > 15 ) && ( DEV_TYPB < 18 ) && (retc == 13)) {
	bProgB = notifyDataB[3];
...
void mCsStatus() {
	...
	strcat(cStatusC,tmpvar);
	strcat(cStatusC,"}");    
	} else if (( DEV_TYPC > 15 ) && ( DEV_TYPC < 18 ) && (retc == 13)) {
	bProgC = notifyDataC[3];
...
//	PRgbBA = RgbBA;
	...
	if (!fcommtp) strcat(ldata,"/rsp");
	strcat(ldata,"/lock");
	if (!bModProgA) esp_mqtt_client_publish(mqttclient, ldata, strOFF, 0, 1, 1);
        else esp_mqtt_client_publish(mqttclient, ldata, strON, 0, 1, 1);
	r4sppcoma = 255;
	t_ppcoma_us = t_mqt_us;
...
//	PRgbBB = RgbBB;
	...
	if (!fcommtp) strcat(ldata,"/rsp");
	strcat(ldata,"/lock");
	if (!bModProgB) esp_mqtt_client_publish(mqttclient, ldata, strOFF, 0, 1, 1);
        else esp_mqtt_client_publish(mqttclient, ldata, strON, 0, 1, 1);
	r4sppcomb = 255;
	t_ppcomb_us = t_mqt_us;
...
//	PRgbBC = RgbBC;
	...
	if (!fcommtp) strcat(ldata,"/rsp");
	strcat(ldata,"/lock");
	if (!bModProgC) esp_mqtt_client_publish(mqttclient, ldata, strOFF, 0, 1, 1);
        else esp_mqtt_client_publish(mqttclient, ldata, strON, 0, 1, 1);
	r4sppcomc = 255;
	t_ppcomc_us = t_mqt_us;
...
//	ESP_LOGI(AP_TAG,"DATA=%.*s", event->data_len, event->data);
	if((event->data_len) && (event->data_len < 64) && (event->topic_len) && (event->topic_len < 64)) {
	t_mqt_us = esp_timer_get_time();
	int topoffa = 0;
	int topoffb = 0;
	int topoffc = 0;
...
//ESP_LOGI(AP_TAG,"Mqtt_delay_C=%d", fdeltmp);
	...
	if (topoffa && DEV_TYPA && ((t_mqt_us - t_ppcona_us) > 3000000)) {
...
//	bprevStateA = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 39) && (temp < 91) && ((!fcommtp) || (!r4sppcoma) || (temp != bHtempA))) {
	r4slppar1a = temp;
	r4slpcoma = 3;
...
//	ESP_LOGI(AP_TAG,"MQTT_CMD_OFF");
	...
	if ((!incascmp("1",event->data,event->data_len)) || (!incascmp("on",event->data,event->data_len))
		|| (!incascmp("true",event->data,event->data_len))) {
	if ((!bModProgA) || (!fcommtp) || (!r4sppcoma) || (inccmp(strON,event->data,event->data_len))) {	
	r4slppar1a = 0;
	r4slpcoma = 8;
	}
//	ESP_LOGI(AP_TAG,"MQTT_lock_ON");
		...
		|| (!incascmp("false",event->data,event->data_len))) {
	if ((bModProgA) || (!fcommtp) || (!r4sppcoma) || (inccmp(strOFF,event->data,event->data_len))) {	
	r4slppar1a = 0;
	r4slpcoma = 7;
	}	
...
// for RMC-903s
	...
	if ((prog < 13) && ((!fcommtp) || (!r4sppcoma) || (prog != bProgA))) {
	r4slppar1a = prog;
	r4slpcoma = 11;
	}
...
//	bprevStateB = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 39) && (temp < 91) && ((!fcommtp) || (!r4sppcomb) || (temp != bHtempB))) {
	r4slppar1b = temp;
	r4slpcomb = 3;
...
//	ESP_LOGI(AP_TAG,"MQTT_CMD_OFF");
	...
	if ((!incascmp("1",event->data,event->data_len)) || (!incascmp("on",event->data,event->data_len))
		|| (!incascmp("true",event->data,event->data_len))) {
	if ((!bModProgB) || (!fcommtp) || (!r4sppcomb) || (inccmp(strON,event->data,event->data_len))) {	
	r4slppar1b = 0;
	r4slpcomb = 8;
	}
//	ESP_LOGI(AP_TAG,"MQTT_lock_ON");
		...
		|| (!incascmp("false",event->data,event->data_len))) {
	if ((bModProgB) || (!fcommtp) || (!r4sppcomb) || (inccmp(strOFF,event->data,event->data_len))) {	
	r4slppar1b = 0;
	r4slpcomb = 7;
	}	
//	ESP_LOGI(AP_TAG,"MQTT_lock_OFF");
...
// for RMC-903s
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t prog = atoi(tbuff);
	if ((prog < 13) && ((!fcommtp) || (!r4sppcomb) || (prog != bProgB))) {
	r4slppar1b = prog;
	r4slpcomb = 11;
	}
...
//	bprevStateC = 255;
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t temp = atoi(tbuff);
	if  ((temp > 39) && (temp < 91) && ((!fcommtp) || (!r4sppcomc) || (temp != bHtempC))) {
	r4slppar1c = temp;
	r4slpcomc = 3;
...
//	ESP_LOGI(AP_TAG,"MQTT_CMD_OFF");
	...
	if ((!incascmp("1",event->data,event->data_len)) || (!incascmp("on",event->data,event->data_len))
		|| (!incascmp("true",event->data,event->data_len))) {
	if ((!bModProgC) || (!fcommtp) || (!r4sppcomc) || (inccmp(strON,event->data,event->data_len))) {	
	r4slppar1c = 0;
	r4slpcomc = 8;
	}
//	ESP_LOGI(AP_TAG,"MQTT_lock_ON");
		...
		|| (!incascmp("false",event->data,event->data_len))) {
	if ((bModProgC) || (!fcommtp) || (!r4sppcomc) || (inccmp(strOFF,event->data,event->data_len))) {	
	r4slppar1c = 0;
	r4slpcomc = 7;
	}	
//	ESP_LOGI(AP_TAG,"MQTT_lock_OFF");
...
// for RMC-903s
	...
	mystrcpy(tbuff, event->data, tempsz);
	uint8_t prog = atoi(tbuff);
	if ((prog < 13) && ((!fcommtp) || (!r4sppcomc) || (prog != bProgC))) {
	r4slppar1c = prog;
	r4slpcomc = 11;
	}
...
static esp_err_t pmain_get_handler(httpd_req_t *req)
{
	...
	strcat(bsend,"<tr><td>ILI9341 320*240 LCD</td><td>");
        (tft_conn)? strcat(bsend,"Connected") : strcat(bsend,"Disconnected");
#endif
...
static esp_err_t pcfgdev1_get_handler(httpd_req_t *req)
{
	...
	if ((DEV_TYPA > 15) && (DEV_TYPA < 64)) strcat(bsend," Cooker");
...
// for RMC-800s
	if (bProgA == 1) strcat(bsend,"selected ");
        strcat(bsend,"value=\"1\">Rice / –ис  рупы (0)</option><option ");
	if (bProgA == 2) strcat(bsend,"selected ");
        strcat(bsend,"value=\"2\">Slow Cooking / “омление (0)</option><option ");
	if (bProgA == 3) strcat(bsend,"selected ");
        strcat(bsend,"value=\"3\">Pilaf / ѕлов (0)</option><option ");
	if (bProgA == 4) strcat(bsend,"selected ");
        strcat(bsend,"value=\"4\">Frying / ∆арка (1-3)</option><option ");
	if (bProgA == 5) strcat(bsend,"selected ");
        strcat(bsend,"value=\"5\">Stewing / “ушение (1-3)</option><option ");
	if (bProgA == 6) strcat(bsend,"selected ");
        strcat(bsend,"value=\"6\">Pasta / ћакароны (0)</option><option ");
	if (bProgA == 7) strcat(bsend,"selected ");
        strcat(bsend,"value=\"7\">Milk Porridge / ћолочна€ каша (0)</option><option ");
	if (bProgA == 8) strcat(bsend,"selected ");
        strcat(bsend,"value=\"8\">Soup / —уп (0)</option><option ");
	if (bProgA == 9) strcat(bsend,"selected ");
        strcat(bsend,"value=\"9\">Yogurt / …огурт (0)</option><option ");
	if (bProgA == 10) strcat(bsend,"selected ");
        strcat(bsend,"value=\"10\">Baking / ¬ыпечка (0)</option><option ");
	if (bProgA == 11) strcat(bsend,"selected ");
        strcat(bsend,"value=\"11\">Steam / ѕар (1-3)</option><option ");
	if (bProgA == 12) strcat(bsend,"selected ");
	strcat(bsend,"value=\"12\">Hot / ¬арка Ѕобовые (0)</option></select>Select program</br>");
	strcat(bsend,"<select name=\"smod\"><option ");
	if (bModProgA == 0) strcat(bsend,"selected ");
        strcat(bsend,"value=\"0\">Not defined (0)</option><option ");
	if (bModProgA == 1) strcat(bsend,"selected ");
        strcat(bsend,"value=\"1\">Vegetables / ќвощи (1)</option><option ");
	if (bModProgA == 2) strcat(bsend,"selected ");
        strcat(bsend,"value=\"2\">Fish / –ыба (2)<option ");
	if (bModProgA == 3) strcat(bsend,"selected ");
	strcat(bsend,"value=\"3\">Meat / ћ€со (3)</option></select>Select program mode</br>");
	} else if ( DEV_TYPA == 17 ) {
// for RMC-903s
	if (bProgA == 1) strcat(bsend,"selected ");
        strcat(bsend,"value=\"1\">Milk Porridge / ћолочна€ каша</option><option ");
	if (bProgA == 2) strcat(bsend,"selected ");
        strcat(bsend,"value=\"2\">Stewing / “ушение</option><option  ");
	if (bProgA == 3) strcat(bsend,"selected ");
        strcat(bsend,"value=\"3\">Frying / ∆арка</option><option ");
	if (bProgA == 4) strcat(bsend,"selected ");
        strcat(bsend,"value=\"4\">Soup / —уп</option><option ");
	if (bProgA == 5) strcat(bsend,"selected ");
        strcat(bsend,"value=\"5\">Steam / ѕар</option><option ");
	if (bProgA == 6) strcat(bsend,"selected ");
        strcat(bsend,"value=\"6\">Pasta / ћакароны</option><option ");
	if (bProgA == 7) strcat(bsend,"selected ");
        strcat(bsend,"value=\"7\">Slow Cooking / “омление</option><option ");
	if (bProgA == 8) strcat(bsend,"selected ");
        strcat(bsend,"value=\"8\">Hot / ¬арка</option><option ");
	if (bProgA == 9) strcat(bsend,"selected ");
        strcat(bsend,"value=\"9\">Baking / ¬ыпечка</option><option ");
	if (bProgA == 10) strcat(bsend,"selected ");
        strcat(bsend,"value=\"10\">Groats /  рупы</option><option ");
	if (bProgA == 11) strcat(bsend,"selected ");
        strcat(bsend,"value=\"11\">Pilaf / ѕлов</option><option ");
	if (bProgA == 12) strcat(bsend,"selected ");
        strcat(bsend,"value=\"12\">Yogurt / …огурт</option><option ");
	if (bProgA == 13) strcat(bsend,"selected ");
        strcat(bsend,"value=\"13\">Pizza / ѕицца</option><option ");
	if (bProgA == 14) strcat(bsend,"selected ");
        strcat(bsend,"value=\"14\">Bread / ’леб</option><option ");
	if (bProgA == 15) strcat(bsend,"selected ");
	strcat(bsend,"value=\"15\">Desserts / ƒесерты</option></select>Select program</br>");
	strcat(bsend,"<select name=\"smod\"><option ");
...
static esp_err_t pcfgdev2_get_handler(httpd_req_t *req)
{
	...
	if ((DEV_TYPB > 15) && (DEV_TYPB < 64)) strcat(bsend," Cooker");
...
static esp_err_t pcfgdev3_get_handler(httpd_req_t *req)
{
	...
	if ((DEV_TYPC > 15) && (DEV_TYPC < 64)) strcat(bsend," Cooker");
...
static esp_err_t psetting_get_handler(httpd_req_t *req)
{
	...
	if (DEV_TYPA == 2) strcat(bsend,"selected ");
        strcat(bsend,"value=\"2\">RK-G200(old)</option><option ");
	if (DEV_TYPA == 3) strcat(bsend,"selected ");
	strcat(bsend,"value=\"3\">RK-G200S</option><option ");
	...
	strcat(bsend,"> flip 180&deg; <input name=\"pnmiso\" type=\"number\" value=\"");
...
// every 1s display time and date
	...
	blstnum_inc();
	t_tinc_us = t_before_us;	
        if (REQ_NAMEA[0] && NumConnA && !btauthoriza) {
	if (floop) floop--;
	} else if (REQ_NAMEB[0] && NumConnB && !btauthorizb) {
	if (floop) floop--;
	} else if (REQ_NAMEC[0] && NumConnC && !btauthorizc) {
	if (floop) floop--;
	} else floop = 16;
