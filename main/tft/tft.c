...
//#define CS_L GPIO.out_w1tc = (1 << PIN_NUM_CS)//;GPIO.out_w1tc = (1 << PIN_NUM_DC)
//#define CS_H GPIO.out_w1ts = (1 << PIN_NUM_CS)//;GPIO.out_w1ts = (1 << PIN_NUM_DC)
#define CS_L gpio_set_level(PIN_NUM_CS, 0);
#define CS_H gpio_set_level(PIN_NUM_CS, 1);
...
uint32_t lcd_get_sdr(spi_device_handle_t spi)
{
//get_sdr cmd
	CS_L;
	lcd_cmd(spi, 0x0f);
...
///Disable backlight
	...
	if ( lcd_id != 0 ) {
        ESP_LOGI(AP_TAG, "ILI9341 TFT not found.");
	} else {
        //zero, ili
	...
	uint32_t lcd_id = lcd_get_sdr(spi);
	if ( lcd_id == 0xc0) {
	///Enable backlight
	fillScreen(0);
	...
	result = true;
	} else ESP_LOGI(AP_TAG, "ILI9341 TFT not found.");
	}
	return result;
}
...
//Initialize the SPI bus
	ret=spi_bus_initialize(SPI_PORT, &buscfg, DMA_CHAN);
	ESP_ERROR_CHECK(ret);
...
//  tfstate(" Starting R4S Gateway 2020.07.25");
	fillRect(0,224,320,16,TFT_BLACK);
...
void tftclock()
{
	...
	mystrcpy(sday,strftime_buf,4);
	strcat(sday,"  ");
//	mystrcpy(sdate,&strftime_buf[22],2);
//	mystrcpy(&sdate[2],&strftime_buf[3],7);
	mystrcpy(sdate,&strftime_buf[3],7);
	strcat(sdate,"  ");
	setTextColor(TFT_WHITE, TFT_BLACK);
	drawString(stime,0,0,7);
	setTextColor(TFT_GREEN, TFT_BLACK);
	drawString(sday,240,26,4);
	drawString(sdate,220,0,4);
	setTextColor(TFT_WHITE, TFT_BLACK);
}
...
void tfblestate()
{
	...
	if ((MQTT_VALP1[0]) || (MQTT_VALP3[0]) || (MQTT_VALP5[0])) {
	setTextColor(TFT_GREEN, TFT_BLACK);
  	uint32_t sumx = 0;
	uint32_t sumy = 0;
	if (MQTT_VALP1[0]) {
	...
	sumy = sumx;
        sumx += drawString(MQTT_VALP2,sumx,mpos,2);
  	sumx += drawString("%  ", sumx, mpos, 2);
	fillRect(sumy, mpos+16,sumx-sumy,10,TFT_BLACK);
	} else sumx += drawString("' ", sumx, mpos, 4);
	}
	if (MQTT_VALP3[0]) {
	if (!MQTT_VALP4[0]) setTextColor(TFT_GREEN, TFT_BLACK);
        else if ((!incascmp("0",MQTT_VALP4,1)) || (!incascmp("off",MQTT_VALP4,3))||
        (!incascmp("false",MQTT_VALP4,5))) {
	setTextColor(TFT_BLUE, TFT_BLACK);
	} else setTextColor(TFT_RED, TFT_BLACK);
        sumx += drawString(MQTT_VALP3,sumx, mpos,4);
  	sumx += drawString("'  ", sumx, mpos, 4);
	}
	if (MQTT_VALP5[0]) {
	setTextColor(TFT_YELLOW, TFT_BLACK);
        sumx += drawString(MQTT_VALP5,sumx,mpos,4);
	if (MQTT_VALP6[0]) {
  	sumx += drawString("'/", sumx, mpos, 4);
	fillRect(sumx, mpos,30,26,TFT_BLACK);
	sumy = sumx;
        sumx += drawString(MQTT_VALP6,sumx, mpos,2);
  	sumx += drawString("%", sumx, mpos, 2);
	fillRect(sumy, mpos+16,sumx-sumy,10,TFT_BLACK);
	} else sumx += drawString("' ", sumx, mpos, 4);
	}
	...
	if (btauthoriza) {
	if (!bStateA) setTextColor(TFT_BLUE, TFT_BLACK);
	else if (bStateA == 254) setTextColor(TFT_DARKGREY, TFT_BLACK);
	else if ((DEV_TYPA == 16) && (bStateA == 1)) setTextColor(TFT_WHITE, TFT_BLACK);
	else if ((DEV_TYPA == 16) && (bStateA == 5)) setTextColor(TFT_YELLOW, TFT_BLACK);
	else setTextColor(TFT_RED, TFT_BLACK);
	...
	if (bCtempA > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bCtempA > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	...
	if (bHtempA > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bHtempA > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	...
	sumx = drawString("Mqtt, ", 0, 224, 2);
	if (btauthorizb) {
	if (!bStateB) setTextColor(TFT_BLUE, TFT_BLACK);
	else if (bStateB == 254) setTextColor(TFT_DARKGREY, TFT_BLACK);
	else if ((DEV_TYPB == 16) && (bStateB == 1)) setTextColor(TFT_WHITE, TFT_BLACK);
	else if ((DEV_TYPB == 16) && (bStateB == 5)) setTextColor(TFT_YELLOW, TFT_BLACK);
	else setTextColor(TFT_RED, TFT_BLACK);
	...
	sumx += drawString(" Temp: ", sumx, 224, 2);
	if (bCtempB > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bCtempB > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	...
	sumx += drawString("'", sumx, 224, 2);
        setTextColor(TFT_GREEN, TFT_BLACK);
	sumx += drawString(", Heat: ", sumx, 224, 2);
	if (bHtempB > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bHtempB > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	...
	sumx = drawString("Mqtt, ", 0, 224, 2);
	if (btauthorizc) {
	if (!bStateC) setTextColor(TFT_BLUE, TFT_BLACK);
	else if (bStateC == 254) setTextColor(TFT_DARKGREY, TFT_BLACK);
	else if ((DEV_TYPC == 16) && (bStateC == 1)) setTextColor(TFT_WHITE, TFT_BLACK);
	else if ((DEV_TYPC == 16) && (bStateC == 5)) setTextColor(TFT_YELLOW, TFT_BLACK);
	else setTextColor(TFT_RED, TFT_BLACK);
	...
	sumx += drawString(" Temp: ", sumx, 224, 2);
	if (bCtempC > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bCtempC > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
	...
	sumx += drawString("'", sumx, 224, 2);
        setTextColor(TFT_GREEN, TFT_BLACK);
	sumx += drawString(", Heat: ", sumx, 224, 2);
	if (bHtempC > 80) setTextColor(TFT_WHITE, TFT_BLACK);
	else if (bHtempC > 60) setTextColor(TFT_YELLOW, TFT_BLACK);
...
static uint16_t infunc(JDEC *decoder, uint8_t *buf, uint16_t len)
{
    ...
    if ((buf != NULL) && (jd->inPos + len < MyJPGbuflen)) {
