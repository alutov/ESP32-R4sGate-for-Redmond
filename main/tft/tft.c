...
void tftclock()
{
	...
	if (!btauthoriza) sym = TFT_DARKGREY;
	else if (!bStateA && bHeatA) sym = TFT_YELLOW;
	else if (!bStateA) sym = TFT_BLUE;
	else if (bStateA == 254) sym = TFT_DARKGREY;
	else if ((DEV_TYPA > 15) && (DEV_TYPA < 24) && (bStateA == 1)) sym = TFT_WHITE;
	else if ((DEV_TYPA > 15) && (DEV_TYPA < 24) && (bStateA == 5)) sym = TFT_YELLOW;
	else sym = TFT_RED;
	...
	if (!btauthorizb) sym = TFT_DARKGREY;
	else if (!bStateB && bHeatB) sym = TFT_YELLOW;
	else if (!bStateB) sym = TFT_BLUE;
	else if (bStateB == 254) sym = TFT_DARKGREY;
	else if ((DEV_TYPB > 15) && (DEV_TYPB < 24) && (bStateB == 1)) sym = TFT_WHITE;
	else if ((DEV_TYPB > 15) && (DEV_TYPB < 24) && (bStateB == 5)) sym = TFT_YELLOW;
	else sym = TFT_RED;
	setTextColor(sym, bkg);
	...
	if (!btauthorizc) sym = TFT_DARKGREY;
	else if (!bStateC && bHeatC) sym = TFT_YELLOW;
	else if (!bStateC) sym = TFT_BLUE;
	else if (bStateC == 254) sym = TFT_DARKGREY;
	else if ((DEV_TYPC > 15) && (DEV_TYPC < 24) && (bStateC == 1)) sym = TFT_WHITE;
	else if ((DEV_TYPC > 15) && (DEV_TYPC < 24) && (bStateC == 5)) sym = TFT_YELLOW;
	else sym = TFT_RED;
	setTextColor(sym, bkg);
...
void blstnum_inc() {
	...
	else if (REQ_NAMEA[0] || REQ_NAMEB[0] || REQ_NAMEC[0]) {
	int i = 0;
	while (i < 3) {
	blstnum++;
...
void tfblestate()
{
	...
	char buff[16];
	int  mpos = 214;
	if (f_update || REQ_NAMEA[0] || REQ_NAMEB[0] || REQ_NAMEC[0]) mpos = 198;
